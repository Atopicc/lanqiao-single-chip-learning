...
void main()
{
  ...
  while(1)
  {
    ...
    {
      gFlag_10ms = 0;
      if(Uart_Rev_DelayTimees)
      {
        Uart_Rev_DelayTimees--;
        if(Uart_Rev_DelayTimees == 0)
        {
          if(Uart_Rev_Num == 6)
          {
            PC_CMD_Flag = 1;
            for(i=0;i<6;i++)
            {
              if(Uart_CMD1_Data[i] != Uart_Rev_Data[i]) PC_CMD_Flag = 0;
            }
            if(PC_CMD_Flag) externaldevice_IO(0,1);
            PC_CMD_Flag = 1;
            for(i=0;i<6;i++)
            {
              if(Uart_CMD2_Data[i] != Uart_Rev_Data[i]) PC_CMD_Flag = 0;
            }
            if(PC_CMD_Flag) externaldevice_IO(0,0);
          }
          for(i=0;i<20;i++)
          {
            Uart_Rev_Data[i] = 0;
      ...
      dispbuf[5] = AD1_value_Now/100;
      dispbuf[6] = AD1_value_Now%100/10;
      dispbuf[7] = AD1_value_Now%10;
